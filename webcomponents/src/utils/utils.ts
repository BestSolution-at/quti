...
export function createDateRange(min: CalendarDate, max: CalendarDate): Range<CalendarDate> {
  return {
    min,
    max,
...
export function computeLaneLayout<T extends LaneLayoutEntry<unknown>>( layoutEntries: T[]): LaneLayout<T> {
  let lanes: LaneLayoutEntry<unknown>[][] = [];
  let lastEnding : number | null = null;

  const rv : T[] = layoutEntries
    .sort( (e1, e2) => {
  ...
  return {
    get maxLanes() { return rv.map( e => e.lane.maxLane ).reduce( (a, b) => Math.max(a, b), -Infinity ); },
    entries: rv
  };
}
...
function computeLaneSpan(layoutEntry: LaneLayoutEntry<unknown>, laneIndex: number, columns: LaneLayoutEntry<unknown>[][])
{
  let colSpan = 1;
