...
function fnCreate(
	props: ServiceProps<api.service.ErrorType>,
): api.service.CalendarService['create'] {
...
function fnGet(
	props: ServiceProps<api.service.ErrorType>,
): api.service.CalendarService['get'] {
	const { baseUrl, fetchAPI = fetch, lifecycleHandlers = {} } = props;
	const { preFetch, onSuccess, onError, onCatch, final } = lifecycleHandlers;
...
function fnUpdate(
	props: ServiceProps<api.service.ErrorType>,
): api.service.CalendarService['update'] {
	const { baseUrl, fetchAPI = fetch, lifecycleHandlers = {} } = props;
	const { preFetch, onSuccess, onError, onCatch, final } = lifecycleHandlers;
...
function fnEventView(
	props: ServiceProps<api.service.ErrorType>,
): api.service.CalendarService['eventView'] {
	const { baseUrl, fetchAPI = fetch, lifecycleHandlers = {} } = props;
	const { preFetch, onSuccess, onError, onCatch, final } = lifecycleHandlers;
	return async (
		key: string,
		...
		try {
			...
			const $headers = new Headers($init.headers ?? {});
			$headers.append('Content-Type', 'application/json');
			ifDefined(resultTimeZone, (v) =>
				$headers.append('resultTimeZone', `${v}`),
			);
			$init.headers = $headers;
			...
			if ($response.status === 200) {
				...
				return safeExecute(api.result.ERR(err), () =>
					onError?.('eventView', err),
				);
			} else if ($response.status === 400) {
				const err = {
					_type: 'InvalidArgument',
